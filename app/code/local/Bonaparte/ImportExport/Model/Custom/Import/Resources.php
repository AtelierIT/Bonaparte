...
/**
 * Stores the business logic for the custom resource import
 *
 * @category    Bonaparte
 * @package     Bonaparte_ImportExport
 * @author      Atelier IT Team <office@atelierit.ro>
 */
class Bonaparte_ImportExport_Model_Custom_Import_Resources extends Bonaparte_ImportExport_Model_Custom_Import_Abstract
{
    /**
     *
     *
    ...
    private function _addProduct($productData) {
        ...
        foreach ($productData['Items']['value'] as $productItem){
            ...
            $productSKUCollection = Mage::getModel('catalog/product')->getCollection()
                ->addAttributeToSelect(array('name','sku'))
                ->addAttributeToFilter('sku', array('like' => $productItem['CinoNumber']['value'] . '%'))
                ->load();

            foreach ($productSKUCollection as $productSKUItem){
